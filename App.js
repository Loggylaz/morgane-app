...
export default function MorganeApp() {
  ...
  const addTask = () => {
    ...
    setTasks(t => [...t, { id: Date.now(), text: newTask, done: false, date: getToday() }]);
    setNewTask("");
  };
    ...
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "480px", margin: "0 auto", padding: "16px" }}>
        ...
        {tab === "home" && (<div>
          ...
          <Card>
            <CardTitle right={`${todayTasks.filter(t => t.done).length}/${todayTasks.length}`}>📋 Tâches du jour</CardTitle>
            <div style={{ display: "flex", gap: "6px", marginBottom: "10px" }}>
              <input value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => e.key === "Enter" && addTask()}
                placeholder="Ajouter une tâche..." style={{
        ...
        {tab === "tasks" && (<div>
          ...
          <Card style={{ padding: "14px" }}>
            {todayTasks.length === 0 ? (
              <p style={{ color: "#AEAEB2", textAlign: "center", fontSize: "14px", padding: "20px 0" }}>Rien pour le moment 🌸 Ajoute tes objectifs du jour !</p>
            ) : todayTasks.map(t => (
              <div key={t.id} style={{
